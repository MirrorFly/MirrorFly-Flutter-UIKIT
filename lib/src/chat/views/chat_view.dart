...
import 'package:mirrorfly_uikit_plugin/src/common/widgets.dart';
...
class ChatView extends GetView<ChatController> {
  const ChatView({Key? key,this.profile,required this.onBack}) : super(key: key);
      ...
      children: <Widget>[
        ...
        controller.isAudioRecording.value == Constants.audioRecordInitial
            ? IconButton(
                ...
                icon: SvgPicture.asset(audioMic),
              )
            : const SizedBox.shrink(),
        const SizedBox(
          width: 5,
    ...
    return Obx(() {
      return AppBar(
        ...
        leading: InkWell(
          onTap: () {
            onBack();
            debugPrint('Get.route'+Get.previousRoute);
            if (controller.showEmoji.value) {
              controller.showEmoji(false);
            } else if (controller.nJid != null) {
              // Get.offAllNamed(Routes.dashboard);
