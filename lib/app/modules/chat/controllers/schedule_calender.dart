...
class ScheduleCalender {
  ...
  Future<void> selectCalendarId() async {
    ...
    if (calendarsResult.hasErrors) {
      debugPrint(
          "retrieveCalendars ${calendarsResult.errors.first.errorMessage}");
      return;
    }
    ...
    Calendar? calendar = await customDialog(
  ...
  Future<void> addEvent(MeetChatMessage meetMessage) async {
    ...
    if (exists) return;
    final startTime =
        DateTime.fromMillisecondsSinceEpoch(meetMessage.scheduledDateTime);
    final event = Event(
      calenderId,
      title: meetMessage.title.isEmpty
          ? "Flutter-Mirrorfly-Meet"
          : meetMessage.title,
    ...
    if (result!.isSuccess) {
      ...
      debugPrint(
          "Failed to add event ${event.toJson()} ${result.errors.first.errorMessage}");
    }
  }
  ...
  Future<bool> isEventAlreadyAdded(MeetChatMessage meetMessage) async {
    ...
    final result = await _deviceCalendarPlugin.retrieveEvents(
      ...
      RetrieveEventsParams(
          startDate: startTime.subtract(const Duration(minutes: 1)),
          endDate: endTime.add(const Duration(minutes: 1))),
    );
    ...
    for (Event event in result.data!) {
      if (event.description == meetMessage.link &&
          event.start == toTZDateTime(startTime)) {
