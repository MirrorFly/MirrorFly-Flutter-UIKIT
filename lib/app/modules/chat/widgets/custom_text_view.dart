...
class CustomTextView extends StatelessWidget {
  ...
  Widget getNormalText() {
    var getText = CustomTextViewManager.getCustomText(
        text + mentionUserIds.join(",") + key.toString());
    if (getText != null) {
      return Text.rich(
        getText,
        maxLines: maxLines,
    ...
    var normalTextSpans = TextUtils.getNormalTextSpans(
        key,
        text,
        mentionUserIds,
        defaultTextStyle,
        ...
        : FutureBuilder(
            ...
            builder: (context, data) {
              if (data.connectionState == ConnectionState.done &&
                  data.hasData) {
                var formattedString = MentionUtils.replaceMentionedUserText(
                    key,
                    text,
                    mentionUserIds,
                    data.data!,
  ...
  Widget searchWidget() {
    var span = CustomTextViewManager.getCustomText(
        text + mentionUserIds.join(",") + key.toString());
    if (span != null) {
      var changedSearchSpans = TextUtils.getSearchedTextSpans(
          span.toPlainText(),
      ...
      return mentionUserIds.isNotEmpty ? mentionUserText() : getNormalText();
    }
  }
}
