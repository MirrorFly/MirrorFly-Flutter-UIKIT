...
class GroupParticipantsController extends GetxController {
  ...
  Future<void> initGroupParticipantController({required BuildContext buildContext, required String groupId, required String callType}) async {
        ...
        infoTap: () {
          ...
          if (profile.value.isGroupProfile ?? false) {
            ...
            Navigator.push(
                context,
                MaterialPageRoute(
                    builder: (con) => ChatInfoView(jid: profile.value.jid.checkNull())));
          }
        },
        profile: profile,
        ...
        Mirrorfly.makeGroupVoiceCall(groupJid: groupId.value, toUserJidList: selectedUsersJIDList, flyCallBack: (FlyResponse response) {
          if (response.isSuccess) {
            ...
            MirrorflyUikit.instance.navigationManager.navigateTo(context: context,
                pageToNavigate: OutGoingCallView(userJid: selectedUsersJIDList), routeName: 'outgoing_call_view',
                onNavigateComplete: (){
